(ns foreignsurvey.operation
  "OperationActor -- one foreign-related survey operation = one
  supervised actor run, expressed as a langgraph StateGraph. The advisor
  (ForeignSurvey-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Survey-Integrity Compliance
  ...
  One graph run = one survey operation. No unbounded inner loop.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor.
  `:project/file`/`:survey/field` ALWAYS reach this node when the
  governor is clean -- see `foreignsurvey.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [foreignsurvey.foreignsurveyllm :as foreignsurveyllm]
            [foreignsurvey.governor :as governor]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (foreignsurveyllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (foreignsurveyllm/-advise advisor store request)]
            {:proposal p :audit [(foreignsurveyllm/trace request p)]})))
